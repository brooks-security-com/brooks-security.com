...
4. Create OAuth 2.0 credentials:
   ...
   - Name: "grc-tools"
   - Authorized redirect URIs: `https://auth.brooks-security.com/oauth2/idpresponse`
...
2. New registration:
   ...
   - Redirect URI: Web → `https://auth.brooks-security.com/oauth2/idpresponse`
...
2. You should be redirected to the Cognito login page at `auth.brooks-security.com`
...
User → CloudFront (/grc-tools/*)
       ...
       Lambda@Edge auth-gate
         ...
         └── no auth → redirect to Cognito
                         ↓
                   Cognito Hosted UI (auth.brooks-security.com)
                         ├── Google OAuth
                         └── Microsoft OAuth
```
...
**Redirect loop:** Check that the redirect URI in Cognito matches the OAuth app configuration. The Cognito redirect URI is `https://auth.brooks-security.com/oauth2/idpresponse`.
