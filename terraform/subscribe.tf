...
resource "aws_iam_role_policy" "subscribe_lambda" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      {
        # The new subscribe params plus the existing reCAPTCHA and Notion params.
        Sid    = "ReadParams"
        Effect = "Allow"
...
resource "aws_lambda_function" "subscribe" {
  ...
  description   = "Subscribe form: reCAPTCHA Enterprise + shared secret, append row to Google Sheet via WIF, create Prospect in Notion Clients DB (fail-soft)."
  ...
  environment {
    variables = {
      ...
      NOTION_DATABASE_ID           = var.subscribe_notion_database_id
    }
  }
}
